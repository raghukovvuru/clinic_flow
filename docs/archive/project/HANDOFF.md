...
This handoff document reflects an earlier implementation phase and is not the active source of truth for the live codebase.

It predates newer runtime pieces such as patch-managed Healthcare custom fields, `Service Point`, `receptionist-dashboard`, `doctor-workspace-v2`, guardian flow, and emergency intake flow.

Use `CLAUDE.md`, `AGENTS.md`, and `ARCHITECTURE.md` first.
